...
export class VendorOrdersService {
  ...
  // find all.
  async findAll(vendorId: number, findVendorOrdersDto: FindVendorOrdersDto) {
    ...
    const offset = (findVendorOrdersDto.page - 1) * findVendorOrdersDto.limit;
    ...
    if (findVendorOrdersDto.dateFilterOption === DateFilterOption.CUSTOM) {
      ...
      dateRange = Helpers.getDateRangeForFilterOption(
        findVendorOrdersDto.dateFilterOption,
      );
    }
    ...
    const [orders, count] = await mainQueryBuilder
      ...
      .skip(offset)
      .take(findVendorOrdersDto.limit)
      .getManyAndCount();
